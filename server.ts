...
app.post("/api/gemini/story", async (req, res) => {
  ...
  // High-fidelity local storyteller database for graceful offline or API error fallback
  ...
  // Helper to map search terms in case of visual name variations
  const getFallbackStory = (name: string): string => {
...
Adhere strictly to the 3-sentence limit. Do NOT include any intro or outro text (like "Here is the story..."). Produce only the three engaging sentences.`;
    ...
    // Fallback stories are selected instantly and returned with 200 OK so client stays fully hydrated
...
// Skip standalone server when running as a Vercel serverless function
